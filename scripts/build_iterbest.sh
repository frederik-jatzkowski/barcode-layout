...
if [ "$#" -ne 5 ]; then
    if [ "$#" -ne 4 ]; then
        ...
        echo "build_iterbest.sh <NUMBER_OF_BARCODES> <BARCODE_LENGTH> <DIM_X> <DIM_Y> [ <NEIGHBORHOOD_NAME> ]"
        exit 1
    fi
...
cat << EOF > `dirname $0`/../src/iterbest/params.hu
#ifndef _PARAMS_
#define _PARAMS_
...
cat << EOF > `dirname $0`/../src/iterbest/neighborhood.cu
...
nvcc -Xptxas -O3 --debug --ptxas-options=-v -o ./bin/barlay ./src/iterbest/main.cu